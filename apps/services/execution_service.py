...
app = create_service_app("TAOS Execution Service")
router = APIRouter(prefix="/execution", tags=["execution"])

_tool_registry = ToolRegistry()
register_all_builtin_tools(_tool_registry)
_executor = Executor(StepRunner(ToolExecutor(_tool_registry)))
...
async def run_step(req: StepServiceRequest) -> StepServiceResponse:
    ...
    step_result = await _executor.execute_step(
        step=step,
        state=state,
...
async def health() -> dict:
    return {"ok": True, "service": "execution"}
